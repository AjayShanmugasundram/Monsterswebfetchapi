...
title.innerText = ("LETS GO TO THE WORLD OF MONSTERS")
...
select1.setAttribute("placeholder", "Upto 57 Monsters -Type By ID")
...
countryimage.setAttribute("src", "3-31446_monster-hunter-world-wallpaper-hd.jpeg")
...
thankyou.innerHTML=("LET'S GO FOR HUNT")
...
for (i = 0; i < 58; i++) {
    if (i == va) {
        fetch(`https://mhw-db.com/monsters/${i}`)
            .then((d) => d.json())
            .then((data) => {
                document.getElementById("line1").innerHTML = data.id
                document.getElementById("line2").innerHTML = data.species
                document.getElementById("meaning").innerHTML = data.name
